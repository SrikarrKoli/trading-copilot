-- The Edge Function verifies the JWT and permanent owner UUID before invoking
-- this function. The owner foreign key guarantees that the derived UUID exists.
-- This replacement removes a redundant auth.users read, which service_role
-- intentionally cannot perform, without widening privileges or using
-- SECURITY DEFINER.
...
comment on function public.commit_import(
  ...
  'Service-only atomic import. Caller verifies the JWT owner; the owner FK enforces identity existence. Counts are derived from persisted rows.';
